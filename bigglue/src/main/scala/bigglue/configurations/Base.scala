...
object PipeConfig {
   ...
   def liftTypeSafeConfigToLabel(mainConfig: Config, stepName: String): Config = {
     if (!mainConfig.getConfig(Constant.BIGGLUE).hasPath(stepName)) { return mainConfig }
     val stepConfig = mainConfig.getConfig(Constant.BIGGLUE).getConfig(stepName)
     stepConfig.withFallback(mainConfig.getConfig(Constant.BIGGLUE)).atPath(Constant.BIGGLUE)
   }
